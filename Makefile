...
#	@echo "  ${MAKE} openlinux2_2   for OpenLinux 2.2"
	...
	@echo "  ${MAKE} OSX            for Macintosh OS X"
	@echo "  ${MAKE} linux_intel    for Linux with g++ 3.x or above"
	@echo "  ${MAKE} linux_intel-64 for RedHat 5.x & 6.x Linux on 64-bit systems"
...
OSX: prep
	...
	@ln -s ../../include/missing_proto/jMissingProto_empty.h \
           include/jcore/jMissingProto.h
	@${TEST_ACE_CONFIG} config-macosx-panther.h ${CREATE_ACE_CONFIG}
	@${TEST_ACE_MACROS} platform_macosx_panther.GNU ${CREATE_ACE_MACROS}
	@${INSTALL_CMD}
